...
// https://peggyjs.org/
	...
	export var getArgs = (hDesc = undef, hOptions = {}) => {
	  var argStr, args, err, hResult, i, isMissing, key, len, ref, tracer;
	  ...
	  if (defined(hDesc)) {
	    check(hResult, hDesc);
	    isMissing = (key) => {
	      return (key !== '_') && !hasKey(hResult, key) && (hDesc[key].type === 'boolean');
	    };
	    ref = keys(hDesc).filter(isMissing);
...
function peg$parse(input, options) {
  ...
  var peg$startRuleFunctions = { cmdArgs: peg$parsecmdArgs, arg: peg$parsearg, stringVal: peg$parsestringVal, _: peg$parse_ };
