...
// ---------------------------------------------------------------------------
// --- ASYNC !!!
export var getPeggyParser = async(filePath, hOptions = {}) => {
...
export var procPeggy = (contents, hMetaData = {}, filePath = undef, hOptions = {}) => {
  var allCode, allowedStartRules, byteCodeWriter, debug, debugAllCode, debugPreProcess, dumpAST, err, hMeta, hOtherFiles, hResult, i, include, j, jsCode, lInputs, lUses, len, len1, map, opDumper, peggyCode, source, sourceMap, sourceNode, str, text, trace, type;
  ...
  for (i = 0, len = lUses.length; i < len; i++) {
    filePath = lUses[i];
    ({
      hMetaData: hMeta,
      contents: str
  ...
  if (debug) {
    console.log("INPUTS:");
    allCode = '';
    for (j = 0, len1 = lInputs.length; j < len1; j++) {
      ({source, text} = lInputs[j]);
